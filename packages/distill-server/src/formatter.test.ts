...
describe("formatConversation", () => {
  ...
  it("日本語タイトルを処理する", () => {
    const result = formatConversation(makeData({ title: "プロジェクト設計相談" }), "2026-03-02");
    expect(result).toContain('title: "プロジェクト設計相談"');
    expect(result).toContain("# プロジェクト設計相談");
  });

  it("タイトル内のダブルクォートをエスケープする", () => {
    const result = formatConversation(makeData({ title: 'Say "hello"' }), "2026-03-02");
    expect(result).toContain('title: "Say \\"hello\\""');
  });

  it("メッセージが空の場合でも動作する", () => {
    const result = formatConversation(makeData({ messages: [] }), "2026-03-02");
